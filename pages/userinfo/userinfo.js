...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    var loginUser = app.globalData.login_user_id;
    var id = options.id
    if(id == null || id == ''){
        id = loginUser;
        that.setData({
    ...
    //用户发布图片的總數
    wx.request({
      url: Data.getUrl() + '/photo/searchpcount',
      method: 'GET',
      data: { uid: id },
      header: {
    ...
    //用户发布视频的總數
    wx.request({
      url: Data.getUrl() + '/video/searchvcount',
      method: 'GET',
      data: { uid: id },
      header: {
        'Content-Type': 'application/x-www-form-urlencoded'
    ...
    //用戶收藏的總數
    wx.request({
      url: Data.getUrl() + '/collect/searchfcount',
      method: 'GET',
      data: { uid: id },
      header: {
        'Content-Type': 'application/x-www-form-urlencoded'
    ...
    //用户所有信息和他发布的所有作品
    wx.request({
      //url: Data.getUrl() + '/Api/Userinfo/searchinfo',
      method: 'POST',
      data: { id:id },
      header: {
        'Content-Type': 'application/x-www-form-urlencoded'
      ...
      success: function (res) {
        ...
        //用戶标签
        wx.request({
          url: Data.getUrl() + '/Api/Userinfo/sreachlabellimit',
          method: 'POST',
          data: { id: id },
          header: {
            'Content-Type': 'application/x-www-form-urlencoded'
        ...
        //用戶收藏
        wx.request({
          url: Data.getUrl() + '/Api/Userinfo/sreachUserFavorite',
          method: 'POST',
          data: { id:id },
          header: {
            'Content-Type': 'application/x-www-form-urlencoded'
      ...
      //遍历用户发布的所有视频
        ...
        if(user == ''){
          ...
          that.setData({
            ...
            address: user.country + ' ' + user.province,
            sex: user.genderStr,
            age: "",
  ...
  onShow: function () {
    ...
    if (this.data.user_id == app.globalData.login_user_id){
  ...
  attenPerson(e) {
    ...
    if (that.data.fansShow == false) {
      ...
      wx.request({
        url: Data.getUrl() + '/Api/Concern/searchCon',
        method: 'POST',
        data: {
          userid:userid,
          to_userid: to_userid,
